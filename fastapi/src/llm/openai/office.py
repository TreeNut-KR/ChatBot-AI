'''
파일은 OpenAIOfficeModel, office_config.OfficePrompt 등등 클래스를 정의하고 OpenAI API를 사용하여,
...
def build_openai_messages(character_info: office_config.OfficePrompt) -> list:
    """
    캐릭터 정보와 대화 기록을 포함한 OpenAI API messages 형식 생성

    Args:
        character_info (office_config.OfficePrompt): 캐릭터 정보
...
class OpenAIOfficeModel:
    ...
    def generate_response(self, input_text: str, search_text: str, chat_list: List[Dict]) -> str:
        ...
        try:
            ...
            self.character_info: office_config.OfficePrompt = office_config.OfficePrompt(

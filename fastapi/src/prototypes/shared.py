...
group.add_argument('--gradio-auth', type=str, help='Set Gradio authentication password in the format "username:password". Multiple credentials can also be supplied with "u1:p1,u2:p2".', default=None)
...
def do_cmd_flags_warnings():
    ...
    for k in deprecated_args:
        if k in provided_arguments:
            print(f'Warning: The --{k} flag has been deprecated and will be removed soon. Please remove that flag.')
    ...
    if 'COLAB_GPU' not in os.environ and not args.nowebui:
        if args.share:
            print("Warning: The gradio \"share link\" feature uses a proprietary executable to create a reverse tunnel. Use it with care.")
        if any((args.listen, args.share)) and not any((args.gradio_auth, args.gradio_auth_path)):
            print("\nWarning: You are potentially exposing the web UI to the entire internet without any access password.\nYou can create one with the \"--gradio-auth\" flag like this:\n\n--gradio-auth username:password\n\nMake sure to replace username:password with your own.")
            if args.multi_user:
                print('\nWarning: The multi-user mode is highly experimental and should not be shared publicly.')
...
def load_user_config():
    '''
    사용자 모델 설정을 로드하고 GPU 성능에 맞게 자동 설정
    '''
    config_path=Path(f'{args.model_dir}/config-user.yaml')
    
    # GPU 정보 가져오기
    if torch.cuda.is_available():
        gpu1_props=torch.cuda.get_device_properties(1)  # GPU1 정보
        gpu1_vram_gb=gpu1_props.total_memory / (1024**3)  # VRAM in GB
        gpu1_compute=gpu1_props.major + gpu1_props.minor/10  # Compute capability
        
        # 기본 설정
        auto_config={
            'Llama-3-Lumimaid-8B': {
                'loader': 'ExLlamav2_HF',
                'gpu_split': f'{gpu1_vram_gb:.1f}',
                'max_seq_len': min(4096, int(gpu1_vram_gb * 256)),  # VRAM 기반 시퀀스 길이 조정
                'cache_type': 'q4_0' if gpu1_vram_gb < 8 else 'fp16',
                'threads': min(8, os.cpu_count() or 4),
                'n_gpu_layers': -1 if gpu1_vram_gb >= 6 else int(gpu1_vram_gb * 2),
                'no_flash_attn': gpu1_compute < 7.5,  # Flash Attention은 Ampere(8.0) 이상 필요
            }
        }
        ...
        if config_path.exists():
            with open(config_path, 'r', encoding='utf-8') as f:
                ...
                user_config=yaml.safe_load(file_content) if file_content else {}
        else:
            user_config={}

        # 자동 설정과 기존 설정 병합
        for model_name, settings in auto_config.items():
            if model_name not in user_config:
                user_config[model_name]={}
            user_config[model_name].update(settings)
